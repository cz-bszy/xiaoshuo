"""Scan chapters for hard-state contradictions."""
...
def main():
    parser = argparse.ArgumentParser(description="Scan contradictions using invariants.")
    parser.add_argument("--chapters-dir", type=str, default="../chapters", help="Chapters root directory")
    parser.add_argument("--dry-run", action="store_true", help="Do not persist any state changes")
    ...
    for chapter_num, path in chapters:
        ...
        if issues and any(issue.get("severity") == "error" for issue in issues):
            continue

        if not args.dry_run:
            manager.commit(chapter_num, updates, issues, persist=False)

    report_path = project_path / "state" / "scan_contradictions_report.json"
